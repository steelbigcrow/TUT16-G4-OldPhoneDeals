...
## 缺失/待补齐的功能
- Admin 订单导出与高级过滤：旧版有 `/api/admin/orders/export` (CSV/JSON) 以及 searchTerm/brandFilter 过滤；Spring 版仅支持 userId、起止日期分页。需要在 `/api/admin/orders` 增加 searchTerm/brandFilter，并新增 `/api/admin/orders/export` 支持 CSV/JSON。
- Admin 商品详情：旧版有 GET `/api/admin/phones/:phoneId`；Spring 版只提供列表/更新/禁用/删除，缺单个详情获取。建议新增 GET `/api/admin/phones/{phoneId}` 返回商品+卖家基本信息。
