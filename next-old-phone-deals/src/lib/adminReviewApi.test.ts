import { getAdminReviews, toggleReviewVisibility, deleteReview } from './adminReviewApi';
...
describe('adminReviewApi', () => {
  ...
  describe('getAdminReviews', () => {
    it('使用默认分页并返回数据', async () => {
      const token = 'tk';
      const pageData = { content: [{ id: 'r1' }], currentPage: 0, totalPages: 1, total: 1, pageSize: 10 };
